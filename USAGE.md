...
Define tools, receive the model's call requests as typed parts, execute on your side, and send results back. KeyCall normalizes all four wire formats; it never executes a tool and never runs the loop — that stays your code:
...
`result.citations` is a tuple of `Citation(url, title, cited_text)`, normalized across all four provider response formats. Fields the provider didn't supply are `None`.
...
- **`diarize=True` labels each finalized word with its speaker** on AssemblyAI and Deepgram, the two providers whose streaming wire reports one. `word.speaker` carries the provider's own label verbatim (AssemblyAI letters, `"A"` and `"B"`; Deepgram integer indexes as strings, `"0"` and `"1"`), meaningful within one session and never across providers or across a reconnect. ElevenLabs refuses the flag before the socket opens: its realtime words carry a `speaker_id` the provider leaves null on every word (live-verified 2026-09-08), so taking the flag would hand back a column of `None` instead of labels. For a stored file, `transcribe(diarize=True)` diarizes on ElevenLabs too.
...
- **Not every model OpenAI lists under transcription serves stored files.** `gpt-live-transcribe` and `gpt-realtime-whisper` are for its realtime socket, and the stored-file endpoint answers them with a bare 404; nothing in the listing distinguishes them, so KeyCall recognises the family from the id and raises `MODEL_NOT_SUITABLE` before the network, pointing at `transcribe_stream()`. The viewer's file-transcription model picker drops the same models. A release probe checks every transcription model the key lists, in both directions, so a family recorded here that starts working shows up as a failure rather than a hidden model.
...
`dictate()` turns a short spoken utterance into both a verbatim transcript and a cleaned rewrite in one round trip, on AssemblyAI:

```python
result = client.dictate(audio=Path("note.wav").read_bytes())

result.verbatim   # the words as spoken, never altered
result.cleaned    # the provider's ready-to-send rewrite (present by default)
result.confidence # overall 0-1 score
for word in result.words:
    word.text, word.confidence   # per-word confidence, no timing
```

It takes audio as 16-bit PCM bytes only, in one short request (up to two minutes). WAV is recognised from its content; for a raw PCM stream pass `media_type="audio/pcm"`. Four optional inputs steer recognition and the rewrite:

```python
result = client.dictate(
    audio=audio,
    context_prompt="A doctor dictating a patient note.",  # describes the subject
    keyterms=["metoprolol", "hypertension"],              # primes specific words
    language="en",                                        # a single ISO code
    cleanup_instruction="Fix punctuation only.",          # overrides the default rewrite
)
```

| Provider | Verbatim | Cleaned rewrite | Per-word confidence | Word timings |
|---|---|---|---|---|
| AssemblyAI | yes | yes, on by default | yes | no |
| Everyone else | no dictation endpoint: refuses toward AssemblyAI | | | |

- **Two outputs from one call.** `verbatim` is the transcript untouched; `cleaned` is the provider's rewrite, which runs by default. When the rewrite fails (a five-second timeout on the provider's side), `cleaned` is `None`, `cleanup_error` carries the reason, a warning is added, and `verbatim` still returns — the transcript is never lost to a rewrite failure.
...
instructions = TextInput(text=huge_stable_prefix, cacheable=True)

with KeyCall(provider="anthropic", api_key=secret) as client:
    result = client.generate_text(
        model="claude-opus-5",
        messages=[
            Message(role="system", content=[instructions]),
            Message(role="user", content=[TextInput(text="the part that changes each call")]),
        ],
    )
```

The reporting side works the same everywhere already: `result.usage.cached_input_tokens` tells you how many input tokens a call read from cache, on every provider, whether or not you ever set `cacheable`. The write side — what `cacheable=True` does — differs by provider, because the providers themselves differ:

- **Anthropic is the one provider where caching doesn't happen at all without this marker.** Setting it adds a `cache_control` breakpoint to that block; a system message with any cacheable part switches from a plain joined string to a block array, one block per part, so only the marked block carries the breakpoint. `cache_ttl_seconds` chooses Anthropic's own two tiers: `300` ("5m", the default, no extra cost) or `3600` ("1h", billed at double the base input rate). Any other value is refused before the network call, since a silently-rounded TTL would silently change what you're billed. **Anthropic's own docs describe this cache as best-effort, with no guaranteed hit rate** — live-verified 2026-08-29: nine identical trials (same exact prefix, 0-20 seconds between calls) hit five times and missed four, with no correlation between that delay and the outcome. When it hits, the reported token count matches the cached block; it doesn't always hit. Budget for the cache as an optimization, not a guarantee, and check `cached_input_tokens` at runtime rather than assuming it.
- **OpenAI already caches automatically, above roughly 1024-2048 input tokens, with no marker at all.** `cacheable=True` opts into OpenAI's *optional* explicit-breakpoint mode on top of that (GPT-5.6 and later), which live-verified 2026-08-29 as more reliable than leaving it to the automatic matcher — every trial hit on the very next call. `cache_ttl_seconds` has no meaning on OpenAI and is silently unused.
- **Every other provider ignores the flag and keeps caching automatically**, the same as it already does with no marker at all: DeepSeek, Moonshot, xAI, and Perplexity all cache without any caller action, and Gemini's *automatic* caching needs none either. Gemini's *explicit* caching is a separate, stateful resource (`cachedContents.create`, referenced by name in a later call, with its own model binding and lifecycle) rather than an inline per-message marker like Anthropic's and OpenAI's, so it isn't wired to this flag; it's a bigger feature of its own to build if a specific need for it shows up.
...
| xAI | `response_format={"type":"json_schema",...,"strict":true}` | yes |
