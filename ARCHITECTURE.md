...
| `_live.py` | Sync/async live full-duplex session sequencing over the same WebSocket wire (OpenAI's gpt-live), a sibling of the realtime session | Perform I/O directly (the transport owns the socket) |
...
Provider identity and wire protocol are separate. The catalog maps thirteen named providers onto nine protocols; the adapter is chosen by protocol, with named overrides for providers whose behavior diverges. The `stt` protocol has no protocol-level adapter: no generic STT-compatible wire exists the way OpenAI-compatible does, so its two providers resolve by name alone and custom targets cannot claim it. The `elevenlabs` and `typesafe` protocols are single-vendor — one provider speaks each wire — and custom targets cannot claim them either, since they can only claim `openai-compatible`:
...
A separate operation from transcription, not a variant of it. `dictate()` sends one multipart POST — a JSON config part beside a wav/pcm audio part — and reads back both a verbatim transcript and a cleaned rewrite from the single response, through `build_dictation_spec`/`parse_dictation_response`. AssemblyAI is the one provider with the endpoint, and it lives on a third host again (`dictation.assemblyai.com`, distinct from both the REST and streaming hosts), so the catalog's dictation operation carries its own `host`, applied through `RequestSpec.host` the same way multi-host service categories are. It stays a distinct operation rather than a `transcribe()` flag because the result shape differs in kind: dictation reports per-word confidence but no timing, so a `DictationWord` carries no `start_ms`/`end_ms` to fill with an invented zero, and it adds a `cleaned` field (the provider's LLM rewrite, on by default) with a `cleanup_error` and a result warning for the case where the five-second rewrite times out while the verbatim transcript still returns. The transport grew a general typed-part multipart body (`RequestSpec.multipart`) for the JSON-config-plus-audio shape, alongside the raw-binary and file-less-form variants the transcription operation already added. Compressed audio refuses in `build_dictation_spec` before the call, naming the accepted forms rather than reaching the provider for a 415, and every provider without the `dictation` capability refuses in `_dictation_gate`, naming AssemblyAI. A capability-drift probe holds the wire to its shape each release by the endpoint's own signal: an unknown config field returns a 400 naming the field.

## Judgments

`judge()` serves TypeSafe's System One wire: one POST carries a state plus a map of typed questions, and the answers come back typed with calibrated probabilities, through `build_judgment_spec`/`parse_judgment_response`. It is not text generation and returns its own `JudgmentResult` rather than an `InvocationResult`: no prose, no finish reason, no streaming, and no sampling parameters exist on the wire at all — the surface takes none rather than accepting and dropping them. Question types are closed dataclasses (`NoulQuestion`, `ChoiceQuestion`, `ScoreQuestion`) so the adapter can build each wire shape without guessing: choice options go as the provider's mapping-of-name-to-description form (a bare list is rejected on the wire), score rubrics as the ordered level list, and the index-keyed legend and probability maps in a score answer are normalized back into rubric order. Pre-flight refuses a choice past the provider's live-enforced 255-option cap; the base gate `_judgment_gate` refuses every provider without the `judgment` capability, naming TypeSafe. The response's `model` field carries the resolved versioned id behind a rolling alias, surfaced as `JudgmentResult.model`, because the provider's model listing is deliberately not treated as exhaustive: it returns rolling aliases only while explicit versioned ids stay callable, so no gate refuses a model for being unlisted, and the decision-model classification comes from the `jev` identifier rule. Verification treats a judgment provider as its own kind of generate proof: with no text models to walk, `run_verify` spends its budget on one minimal Noul per decision candidate (outcome `judged`). Release-suite drift probes hold the wire's edges: the alias still resolves to a versioned id, a pinned versioned id still answers while unlisted, and the organisation-gated `bounding_box` question type still refuses — asserted by status and error type, never by the message sentence, which has already drifted between probes.
...
Live sessions are an additive sibling of the realtime surface, not a replacement, and reuse the same boundaries over the same wire. OpenAI's gpt-live runs full duplex on its own `v1/live/sessions` endpoint, where the caller's audio and the model's audio overlap and the model delegates reasoning and tool use to a separately-billed backend model (OpenAI's Responses delegation). `adapters/_live.py` owns that dialect as a pure translator, turning provider frames into normalized `LiveEvent`s: the caller's own speech transcript, the model's audio and its output transcript, full-duplex barge-in, a running billed-seconds usage event, and a session-ended event carrying the billed duration read off the close. `_live.py` sequences it the way `_realtime.py` does, sync and async differing only in awaits. The `LiveEvent` taxonomy is a provider-neutral superset: it carries `input_transcript_final` and `turn_complete` for a provider that sends them, and gpt-live's audio path sends neither (probe 2026-09-13), so a caller reads the caller transcript as deltas and detects the caller's turn end from the model's reply. The wire is probe-confirmed end to end against gpt-live, and a committed-fixture drift probe holds it to that shape on every release (the release gate is all-or-nothing over every live target); the normalized taxonomy a caller reads is the stable surface. `live_plan` refuses every provider but OpenAI with a typed `UNSUPPORTED_OPERATION`, and the model rides the session-configuration message rather than the URL.
...
`keycall view` starts a localhost-only stdlib HTTP server (`viewer/_server.py`). Credentials stay server-side in a `Registry` (`viewer/_registry.py`) that maps integer target ids to live clients; the browser only ever sends and receives target ids and `TargetView` records. The provider read timeout is a `Registry`-level setting, not fixed at client construction: `/api/settings` rebuilds every loaded key's client with a new value, retiring rather than closing the replaced client so a request already in flight keeps the timeout it started with. Every `/api/*` request requires a per-run token generated fresh at startup and never persisted. The frontend is dependency-free and builds DOM through `textContent`, under a `default-src 'self'` content security policy; the one `innerHTML` write on a text path replays the Playground's own previously-rendered markdown when restoring a saved conversation. `viewer/_traces.py` holds an in-memory, process-lifetime log of request outcomes (timing and status, never prompts or replies) for the Traces tab. Each generation request still carries the whole conversation as `history`, which the server places before the current turn when it builds the provider request; the request/response cycle itself holds nothing between turns. A separate, session-scoped `Conversation` store on the `Registry` (`save_conversation`/`list_conversations`/`get_conversation`/`clear_conversations`, behind `/api/conversations*`) keeps a saved copy of each finished conversation's history and rendered transcript for the History pane, plus an opaque `setup` object for a task whose inputs live outside the transcript (the Judge task's situation and questions), so a conversation and the panel that produced it survive a page reload; it mirrors what the browser already holds rather than replacing it as the source of truth for replay, and like every other in-memory registry state it is gone when the process stops. Per-viewer conveniences that aren't conversations (an unsent Judge draft, which panels are folded) live in the browser's `localStorage` instead, every access guarded so a private window or blocked site data leaves the page working without them.

File transcription in the Playground is one blocking POST (`/api/transcribe/file`) through the same `client.transcribe()` a library caller uses, the shape the video route already has: sync providers answer in the round trip, and the job-shaped provider is polled server-side within the same call, which supplies the timeout that wire requires. The two transcribe tasks share the transcription model category but not one wire, so their model pickers filter on the catalog's per-model wire facts (served in `/api/targets` as `transcription_wires`), and each gates its keys on its own wire's capability flag; speaker labels and link input gate per provider on their own flags the same way. Dictation in the Playground is the same one-blocking-POST shape (`/api/dictate`) through `client.dictate()`, with no model picker at all: the task gates its keys on the provider's `dictation` flag alone, and the browser's recorder already produces the 16 kHz mono WAV the endpoint takes. Judgments follow the same shape (`/api/judge` through `client.judge()`): the browser sends the state and every question in one body, the route builds the typed question classes (a bare options list becomes the wire's mapping form), and the task gates its keys on the `judgment` flag while its model picker filters on the `decision` category — the one task where the flag and the category are different names. The reply leads each scale answer with its most likely level, not the provider's `score`: that score is the odds-weighted average of 0-based list positions, which reads as a position only when the levels run in order, and code can't tell whether levels that aren't all numbers are in order. "Between" is claimed only for two neighbouring levels that share the odds, and an all-numeric scale that doesn't run one way gets a note. The capability map `/api/targets` serves now carries a row for every loaded custom target too, read from that target's own resolved profile, because a custom endpoint absent from the map read as "unknown, serves everything" and was offered for tasks it can't run.
