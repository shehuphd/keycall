...
Live sessions are an additive sibling of the realtime surface, not a replacement, and reuse the same boundaries over the same wire. OpenAI's gpt-live runs full duplex on its own `v1/live/sessions` endpoint, where the caller's audio and the model's audio overlap and the model delegates reasoning and tool use to a separately-billed backend model (OpenAI's Responses delegation). `adapters/_live.py` owns that dialect as a pure translator, turning provider frames into normalized `LiveEvent`s: interim and final transcripts of the caller's own speech, the model's audio and its output transcript, turn completion, full-duplex barge-in, and a session-ended event carrying the billed duration read off the close. `_live.py` sequences it the way `_realtime.py` does, sync and async differing only in awaits. The wire is PROVISIONAL until a live probe runs against gpt-live (the probe needs a funded, gpt-live-1-entitled key, and the release gate is all-or-nothing over every live target); the normalized `LiveEvent` taxonomy a caller reads is the stable surface, and only the provider strings mapping to it are provisional. `live_plan` refuses every provider but OpenAI with a typed `UNSUPPORTED_OPERATION`, and the model rides the session-configuration message rather than the URL.
