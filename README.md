...
That opens a local web app in your browser with your key already loaded: a dashboard that checks it live, a browsable model list with category filters, and a Playground where you can chat, show a model a picture, record a voice message straight from the page, hold a live voice conversation, transcribe your speech live, transcribe an audio file, dictate a short note and get it back cleaned up, attach a PDF, offer it a tool, have it draw you a picture or render a video, or have it speak text aloud. Keys stay in the local server process and never reach the browser.
