...
That opens a local web app in your browser with your key already loaded: a dashboard that checks it live, a browsable model list with category filters, and a Playground where you can chat, show a model a picture, record a voice message straight from the page, hold a live voice conversation, transcribe your speech live, transcribe an audio file, dictate a short note and get it back cleaned up, ask typed judgment questions about a situation, attach a PDF, offer it a tool, have it draw you a picture or render a video, or have it speak text aloud. Keys stay in the local server process and never reach the browser.
...
- **Dictation.** `dictate()` turns a short spoken utterance into both a verbatim transcript and a cleaned rewrite in one round trip on AssemblyAI — 16-bit PCM audio in, `verbatim` and `cleaned` out, with per-word confidence, an optional context prompt, and a keyterm list to prime recognition. A failed rewrite leaves `cleaned` empty and keeps the verbatim transcript; compressed audio refuses before the call. Sync and async, and every provider without the endpoint refuses toward AssemblyAI.
- **Judgments.** `judge()` asks typed questions about a state on TypeSafe and returns typed answers with calibrated probabilities: a yes/no `NoulQuestion` answered as a 0-1 probability, a `ChoiceQuestion` answered with one option plus per-option probabilities, and a `ScoreQuestion` answered as a position on your rubric that can fall between levels. One call carries every question about a state at once, which is far cheaper and faster than a call per question. The result reports the resolved model id (`jev-latest` answers as its current versioned model), and providers without a judgment endpoint refuse toward TypeSafe.
...
| AssemblyAI | stt | verified | streaming transcription, file transcription, and dictation verified |
...
| TypeSafe | typesafe | verified | judgments verified |
...
MIT. See [LICENSE](https://github.com/shehuphd/keycall/blob/main/LICENSE).
